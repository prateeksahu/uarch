...
module registers(
    ...
    generate
        for(i=0; i<8; i=i+1) begin: MMX
            regfile8x8$    REG(.IN0(mmx_wrdata[i*8+:8]),
                            ...
                            .W(mmx_wraddr),
                            .WE(mmx_wrmask_t[i]),
                            .OUT1(mmx_data0[i*8+:8]),
                            .OUT2(mmx_data1[i*8+:8]),
                            .CLOCK(clk));
        end
    endgenerate
	...
	mux4n #(4)	mk2(4'b0001,4'b0010,rm0,rm0,{gpr_rdsize0[1],gpr_rdaddr0[0]},gpr_rdmask0);
	mux2n #(4)  mk3(4'b0011,4'b1111,gpr_rdsize1[0],rm1);
	mux4n #(4)	mk4(4'b0001,4'b0010,rm1,rm1,{gpr_rdsize1[1],gpr_rdaddr1[0]},gpr_rdmask1);
	
	//Generate addresses.
	wire [2:0] rdaddr0,rdaddr1;
	mux2n #(3) mk5({1'b0,gpr_rdaddr0[2:1]},gpr_rdaddr0,gpr_rdsize0[1],rdaddr0);
	mux2n #(3) mk6({1'b0,gpr_rdaddr1[2:1]},gpr_rdaddr1,gpr_rdsize1[1],rdaddr1);
    
	//Clean up and sign extend read data as required.
	wire [31:0] w1,w2,data0,data1;
	mux2n #(32) ma({{24{gpr_data0[7]}},gpr_data0[7:0]},{{24{gpr_data0[15]}},gpr_data0[15:8]},gpr_rdaddr0[0],w1);
	mux4n #(32) mb(gpr_data0,w1,{{16{gpr_data0[15]}},gpr_data0[15:0]},gpr_data0,gpr_rdsize0,data0);
	mux2n #(32) mc({{24{gpr_data1[7]}},gpr_data1[7:0]},{{24{gpr_data1[15]}},gpr_data1[15:8]},gpr_rdaddr1[0],w2);
	...
	mux2n #(4)  mj1(4'b0011,4'b1111,gpr_wrsize[0],wm1);
	mux4n #(4)	mj2(4'b0001,4'b0010,wm1,wm1,{gpr_wrsize[1],gpr_wraddr[0]},gpr_wrmask);
    inv1$    ig1(wg12,gpr_wrmask[0]);
    and2$    ag5(is01,wg12,gpr_wrmask[1]);
	...
	wire [2:0] wraddr;
	mux2n #(3) mg6({1'b0,gpr_wraddr[2:1]},gpr_wraddr,gpr_wrsize[1],wraddr);
    generate
        for(i=0; i<4; i=i+1) begin: GPR
            regfile8x8$    REG(.IN0(gpr_wrdata_t[i*8+:8]),
                            ...
                            .W(wraddr),
                            .WE(gpr_wrmask_t[i]),
                            .OUT1(gpr_data0[i*8+:8]),
                            .OUT2(gpr_data1[i*8+:8]),
                            .CLOCK(clk));
        end
    endgenerate
endmodule    
